    ...
    const HLSL::BoxAttribs Boxes[] =
        {
            ...
            HLSL::BoxAttribs{-0.1f, -0.1f, -0.1f, 0.1f, 0.1f, 0.1f}};
...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    static constexpr int NumInstancesBase = 4;
    static constexpr int NumInstances     = 104;

    static int PrevInstanceCount    = -1;
    const int  CurrentInstanceCount =104;

    bool NeedUpdate = true;

    if (!m_pTLAS || CurrentInstanceCount != PrevInstanceCount)
    {
        PrevInstanceCount = CurrentInstanceCount;

        // Destruir y reconstruir TLAS
        m_pTLAS.Release();

        TopLevelASDesc TLASDesc;
        TLASDesc.Name             = "TLAS";
        TLASDesc.MaxInstanceCount = CurrentInstanceCount;
        ...
        m_pRayTracingSRB->GetVariableByName(SHADER_TYPE_RAY_GEN, "g_TLAS")->Set(m_pTLAS, SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);
        m_pRayTracingSRB->GetVariableByName(SHADER_TYPE_RAY_CLOSEST_HIT, "g_TLAS")->Set(m_pTLAS, SET_SHADER_RESOURCE_FLAG_ALLOW_OVERWRITE);

        NeedUpdate = false; // reconstrucción
    }
    ...
    // Setup instances
    TLASBuildInstanceData Instances[104] = {};
    ...
    Instances[1].InstanceName = "Sphere Number 1 Instance";
    ...
    Instances[2].InstanceName = "Sphere Number 2 Instance";
    ...
    Instances[3].InstanceName = "Sphere Number 3 Instance";
    ...
    static std::string spheresNames[100];

    // Configuración de la cuadrícula
    const int                             gridSize = 11;
    const float                           cellSize = 3.5f;
    std::mt19937                          rng(42); // Semilla fija para resultados consistentes
    std::uniform_real_distribution<float> jitter(-0.4f * cellSize, 0.4f * cellSize);

    for (int i = 0; i < m_NumActiveSpheres; ++i)
    {
        int row = i / gridSize;
        int col = i % gridSize;

        float baseX = (col - gridSize / 2) * cellSize;
        float baseZ = (row - gridSize / 2) * cellSize;

        float jitterX = jitter(rng);
        float jitterZ = jitter(rng);

        float finalX = baseX + jitterX;
        float finalZ = baseZ + jitterZ;

        spheresNames[i] = "Sphere Number " + std::to_string(i + 4) + " Instance";

        std::string name = "Sphere Number " + std::to_string(i + 4) + " Instance";

        Instances[i + NumInstancesBase].InstanceName = spheresNames[i].c_str();

        Instances[i + NumInstancesBase].Transform.SetTranslation(finalX, -5.5f, finalZ);
        if (i % 2 == 0)
        {
            Instances[i + NumInstancesBase].CustomId = 1;
            Instances[i + NumInstancesBase].pBLAS    = m_pProceduralBLAS;
            Instances[i + NumInstancesBase].Mask     = OPAQUE_GEOM_MASK;
        }

        else
        {
            Instances[i + NumInstancesBase].CustomId = 2;
            Instances[i + NumInstancesBase].pBLAS    = m_pCubeBLAS;
            Instances[i + NumInstancesBase].Mask     = OPAQUE_GEOM_MASK;
            Instances[i + NumInstancesBase].Transform.SetTranslation(finalX, -5.5f, finalZ);
            Instances[i + NumInstancesBase].Transform.SetRotation(float3x3::Scale(0.5f, 0.5f, 0.5f).Data());
        }
    }
    ...
    Attribs.InstanceCount = NumInstancesBase + m_NumActiveSpheres;
...
void Tutorial21_RayTracing::CreateSBT()
{
    ...
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Number 1 Instance", PRIMARY_RAY_INDEX, "SpherePrimaryHit");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Number 2 Instance", PRIMARY_RAY_INDEX, "SpherePrimaryHit");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Number 3 Instance", PRIMARY_RAY_INDEX, "SpherePrimaryHit");
    // clang-format on
    ...
    {
        char name[64];
        std::snprintf(name, sizeof(name), "Sphere Number %d Instance", i + 4);
    ...
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Number 1 Instance", SHADOW_RAY_INDEX, "SphereShadowHit");

    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Number 2 Instance", SHADOW_RAY_INDEX, "SphereShadowHit");

    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Number 3 Instance", SHADOW_RAY_INDEX, "SphereShadowHit");
...
void Tutorial21_RayTracing::UpdateUI()
{
    ...
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        ImGui::Separator();
        ImGui::Text("Dynamic Spheres");

        ImGui::SliderInt("Sphere count (ID 1)", &m_NumActiveSpheres, 0, NumSpheres);
    }
    ImGui::End();
}
} // namespace Diligent
